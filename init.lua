...
require('lazy').setup({
...

  {
    ...
    config = function()
      -- Use vim.diagnostic.config for signs (prevents deprecation warnings)
      do
        local icons = {
      ...
      -- Your on-attach mappings/highlights (kept from your config)
      ...
      -- Define servers and their per-server settings
      local servers = {
        -- Keep empty tables if you want defaults
        eslint = {},
        lua_ls = {
        ...
        jsonls = {},
        bashls = {},
        -- Do NOT add tsserver here if you use typescript-tools.nvim
      }

      -- Install tools + servers
      require('mason').setup()
      local ensure_all = vim.tbl_keys(servers)
      vim.list_extend(ensure_all, { 'stylua' }) -- extra tools
    ...
    config = function()
      ...
      cmp.setup {
        ...
        mapping = cmp.mapping.preset.insert {
          ...
          -- -- asdasd
          ['<C-n>'] = cmp.mapping.complete {}, -- Trigger completion with Ctrl+x.
